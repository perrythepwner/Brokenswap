...
var NoCeloProviderError = /*#__PURE__*/function (_Error) {
  _inheritsLoose(NoCeloProviderError, _Error);

  function NoCeloProviderError() {
    var _this;

    _this = _Error.call(this) || this;
    _this.name = _this.constructor.name;
    _this.message = 'No Celo provider was found on window.celo.';
    return _this;
  }

  return NoCeloProviderError;
...
var InjectedConnector = /*#__PURE__*/function (_AbstractConnector) {
  ...
  _proto.activate = function activate() {
    try {
      var _temp5 = function _temp5(_result) {
        ...
        var _temp = function () {
          if (!account) {
            ...
            return Promise.resolve(window.celo.enable().then(function (sendReturn) {
              return sendReturn && parseSendReturn(sendReturn)[0];
            })).then(function (_window$celo$enable$t) {
              account = _window$celo$enable$t;
            });
          }
        }();
      ...
      if (window.celo.on) {
        window.celo.on('chainChanged', _this5.handleChainChanged);
        window.celo.on('accountsChanged', _this5.handleAccountsChanged);
        window.celo.on('close', _this5.handleClose);
        window.celo.on('networkChanged', _this5.handleNetworkChanged);
      }

      if (window.celo.isMetaMask) {
        ;
      ...
      var _temp6 = _catch(function () {
        return Promise.resolve(window.celo.send('eth_requestAccounts').then(function (sendReturn) {
          return parseSendReturn(sendReturn)[0];
        })).then(function (_window$celo$send$the) {
          account = _window$celo$send$the;
        });
      }, function (error) {
        if (error.code === 4001) {
  ...
  _proto.getProvider = function getProvider() {
    try {
      return Promise.resolve(window.celo);
    } catch (e) {
      return Promise.reject(e);
    }
  };
    ...
    try {
      var _temp12 = function _temp12() {
        function _temp9() {
          if (!chainId) {
            try {
              chainId = parseSendReturn(window.celo.send({
                method: 'net_version'
              }));
            } catch (_unused) {
          ...
          if (!chainId) {
            if (window.celo.isDapper) {
              chainId = parseSendReturn(window.celo.cachedResults.net_version);
            } else {
              chainId = window.celo.chainId || window.celo.netVersion || window.celo.networkVersion || window.celo._chainId;
            }
          }
        ...
        var _temp8 = function () {
          if (!chainId) {
            var _temp14 = _catch(function () {
              return Promise.resolve(window.celo.send('net_version').then(parseSendReturn)).then(function (_window$celo$send$the3) {
                chainId = _window$celo$send$the3;
              });
            }, function () {
      ...
      var _temp13 = _catch(function () {
        return Promise.resolve(window.celo.send('eth_chainId').then(parseSendReturn)).then(function (_window$celo$send$the2) {
          chainId = _window$celo$send$the2;
        });
      }, function () {
  ...
  _proto.getAccount = function getAccount() {
    try {
      var _temp20 = function _temp20() {
        function _temp17() {
          if (!account) {
            account = parseSendReturn(window.celo.send({
              method: 'eth_accounts'
            }))[0];
          }
        ...
        var _temp16 = function () {
          if (!account) {
            var _temp22 = _catch(function () {
              return Promise.resolve(window.celo.enable().then(function (sendReturn) {
                return parseSendReturn(sendReturn)[0];
              })).then(function (_window$celo$enable$t2) {
                account = _window$celo$enable$t2;
              });
            }, function () {
      ...
      var _temp21 = _catch(function () {
        return Promise.resolve(window.celo.send('eth_accounts').then(function (sendReturn) {
          return parseSendReturn(sendReturn)[0];
        })).then(function (_window$celo$send$the4) {
          account = _window$celo$send$the4;
        });
      }, function () {
...
export { InjectedConnector, NoCeloProviderError, UserRejectedRequestError };
